/*
 * TIME MANAGEMENT APPLICATION
 * 
 * Done By: Greg Postings 19002634
 * Class: BCA2 G1
 * Module: PROG 2B
 * 
 * POE TASK 1
 * Start Date and Time: 8 August 2021 at 14:25
 * End Date and Time: 20 September 2021 at 15:35
 * 
 * POE TASK 2
 * Start Date and Time: 5 OCtober 2021 at 16:25
 * End Date and Time: 26 OCtober 2021 at 13:50
 */

//Imports
using System;

//Package
namespace GregPostings19002634PROG2BPOE_Task1.CustomClassLibrary
{
    //Class
    public class SemesterInformation
    {
        //////////////////////////////////////////////////////////////
        // These are getters and setters that are used to store
        // information about the user or information that has been
        // entered by the user.
        //////////////////////////////////////////////////////////////

        //Get And Set Methods

        #region Getters And Setters

        //--------------------------------------------------------------------------------------//
        //Get And Set Methods

        /// <summary>
        /// This is used to store the course code of the course
        /// </summary>
        public string CourseCode { get; set; }

        /// <summary>
        /// This is used to store the course name of the course
        /// </summary>
        public string CourseName { get; set; }

        /// <summary>
        /// This is used to store the semester of the course
        /// </summary>
        public int Semester { get; set; }

        /// <summary>
        /// This is used to store the number of weeks there are in each of the semesters of the course
        /// </summary>
        public double NumberOfWeeksInSemester { get; set; }

        /// <summary>
        /// This is used to store the start date of each semester of the course
        /// </summary>
        public DateTime SemesterStartDate { get; set; }

        #endregion

    }
}
//----------------------------------ooo000 END OF FILE 000ooo-----------------------------------//